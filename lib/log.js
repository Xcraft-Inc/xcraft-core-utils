...
exports.decorate = function (mode, prefix, mod, log, maxWidth) {
  ...
  const embedded = /^[a-zA-Z]+ \[[a-zA-Z/.]+\] (?:Verb|Info|Warn|Err):/.test (textNoColor);
  ...
  if (embedded) {
    const beginEmbeddedLength = beginEmbedded.replace (ansiRegex (), '').length;
    const length = beginLength - beginEmbeddedLength;
    const padding = length > 0 ? new Array (length).join (' ') : '';
    output += `${begin}\n`;
    begin = `${padding}${beginEmbedded}`;
  }
