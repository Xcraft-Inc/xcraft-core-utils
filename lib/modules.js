...
function applyOverloads(appDir, appId, variantId, app, builder) {
  if (!variantId) {
    return;
  }
...
exports.extractForEtc = (appDir, appId, variantId, builder = false) => {
  const app = JSON.parse(
    fse.readFileSync(path.join(appDir, appId, 'app.json'))
  );
  applyOverloads(appDir, appId, variantId, app, builder);
  ...
  Object.keys(app.xcraft)
    ...
    .forEach((mod) => {
      ...
      mod.appIds.forEach((appId) => {
        ...
        applyOverloads(appDir, appId, variantId, appConfig, builder);
